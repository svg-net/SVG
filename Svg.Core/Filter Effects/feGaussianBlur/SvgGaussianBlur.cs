...
namespace Svg.FilterEffects
{
    ...
    public class SvgGaussianBlur : SvgFilterPrimitive
    {
        ...
        public Bitmap Apply(Image inputImage)
        {
            ...
            if (bitmapSrc == null) bitmapSrc = Factory.Instance.CreateBitmap(inputImage);//new Bitmap(inputImage);

            using (RawBitmap src = new RawBitmap(bitmapSrc))
            {
                using (RawBitmap dest = new RawBitmap(Factory.Instance.CreateBitmap(inputImage.Width, inputImage.Height)))
                {
