...
namespace Svg.UnitTests
{
    ...
    public class SvgColourConverterTests
    {
        ...
        {
            var aqua = _converter.Parse(null, _cultureInfo, "aqua");
        ...
        {
            var activeBorder = _converter.Parse(null, _cultureInfo, "ActiveBorder");
            Assert.AreEqual(SystemColors.ActiveBorder, activeBorder);

            var activeCaption = _converter.Parse(null, _cultureInfo, "ActiveCaption");
            Assert.AreEqual(SystemColors.ActiveCaption, activeCaption);

            var appWorkspace = _converter.Parse(null, _cultureInfo, "AppWorkspace");
            Assert.AreEqual(SystemColors.AppWorkspace, appWorkspace);

            // TODO: Check if SystemColors.Desktop is valid color for Background.
            var background = _converter.Parse(null, _cultureInfo, "Background");
            Assert.AreEqual(SystemColors.Desktop, background);

            var buttonFace = _converter.Parse(null, _cultureInfo, "ButtonFace");
            Assert.AreEqual(SystemColors.ButtonFace, buttonFace);

            var buttonHighlight = _converter.Parse(null, _cultureInfo, "ButtonHighlight");
            Assert.AreEqual(SystemColors.ButtonHighlight, buttonHighlight);

            var buttonShadow = _converter.Parse(null, _cultureInfo, "ButtonShadow");
            ...
            var buttonText = _converter.Parse(null, _cultureInfo, "ButtonText");
            ...
            var captionText = _converter.Parse(null, _cultureInfo, "CaptionText");
            Assert.AreEqual(SystemColors.ActiveCaptionText, captionText);

            var grayText = _converter.Parse(null, _cultureInfo, "GrayText");
            Assert.AreEqual(SystemColors.GrayText, grayText);

            var highlight = _converter.Parse(null, _cultureInfo, "Highlight");
            Assert.AreEqual(SystemColors.Highlight, highlight);

            var highlightText = _converter.Parse(null, _cultureInfo, "HighlightText");
            Assert.AreEqual(SystemColors.HighlightText, highlightText);

            var inactiveBorder = _converter.Parse(null, _cultureInfo, "InactiveBorder");
            Assert.AreEqual(SystemColors.InactiveBorder, inactiveBorder);

            var inactiveCaption = _converter.Parse(null, _cultureInfo, "InactiveCaption");
            Assert.AreEqual(SystemColors.InactiveCaption, inactiveCaption);

            var inactiveCaptionText = _converter.Parse(null, _cultureInfo, "InactiveCaptionText");
            ...
            var infoBackground = _converter.Parse(null, _cultureInfo, "InfoBackground");
            Assert.AreEqual(SystemColors.Info, infoBackground);

            var infoText = _converter.Parse(null, _cultureInfo, "InfoText");
            Assert.AreEqual(SystemColors.InfoText, infoText);

            var menu = _converter.Parse(null, _cultureInfo, "Menu");
            Assert.AreEqual(SystemColors.Menu, menu);

            var menuText = _converter.Parse(null, _cultureInfo, "MenuText");
            Assert.AreEqual(SystemColors.MenuText, menuText);

            var scrollbar = _converter.Parse(null, _cultureInfo, "ScrollBar");
            ...
            var threeDDarkShadow = _converter.Parse(null, _cultureInfo, "ThreeDDarkShadow");
            ...
            var threeDFace = _converter.Parse(null, _cultureInfo, "ThreeDFace");
            ...
            var threeDHighlight = _converter.Parse(null, _cultureInfo, "ThreeDHighlight");
            ...
            var threeDLightShadow = _converter.Parse(null, _cultureInfo, "ThreeDLightShadow");
            ...
            // var threeDShadow = _converter.Parse(null, _cultureInfo, "ThreeDDarkShadow");
            // Assert.AreEqual(SystemColors.??, threeDShadow);

            var window = _converter.Parse(null, _cultureInfo, "Window");
            Assert.AreEqual(SystemColors.Window, window);

            var windowFrame = _converter.Parse(null, _cultureInfo, "WindowFrame");
            Assert.AreEqual(SystemColors.WindowFrame, windowFrame);

            var windowText = _converter.Parse(null, _cultureInfo, "WindowText");
        ...
        public void ParseReturnsValidColor_grey()
        {
            ...
            var g1 = _converter.Parse(null, _cultureInfo, "grey");
            Assert.AreEqual(Color.Gray, g1);

            var g2 = _converter.Parse(null, _cultureInfo, "gray");
            Assert.AreEqual(Color.Gray, g2);

            var g3 = _converter.Parse(null, _cultureInfo, "Grey");
            Assert.AreEqual(Color.Gray, g3);

            var g4 = _converter.Parse(null, _cultureInfo, "Gray");
            Assert.AreEqual(Color.Gray, g4);

            // lightgray

            var lg1 = _converter.Parse(null, _cultureInfo, "lightgrey");
            Assert.AreEqual(Color.LightGray, lg1);

            var lg2 = _converter.Parse(null, _cultureInfo, "lightgray");
            Assert.AreEqual(Color.LightGray, lg2);

            var lg3 = _converter.Parse(null, _cultureInfo, "LightGrey");
            Assert.AreEqual(Color.LightGray, lg3);

            var lg4 = _converter.Parse(null, _cultureInfo, "LightGray");
            Assert.AreEqual(Color.LightGray, lg4);

            // darkslategray

            var dsg1 = _converter.Parse(null, _cultureInfo, "darkslategrey");
            Assert.AreEqual(Color.DarkSlateGray, dsg1);

            var dsg2 = _converter.Parse(null, _cultureInfo, "darkslategray");
            Assert.AreEqual(Color.DarkSlateGray, dsg2);

            var dsg3 = _converter.Parse(null, _cultureInfo, "DarkSlateGrey");
            Assert.AreEqual(Color.DarkSlateGray, dsg3);

            var dsg4 = _converter.Parse(null, _cultureInfo, "DarkSlateGray");
